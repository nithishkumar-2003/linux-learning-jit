...
#        overlays = [f"overlays/{f}" for f in os.listdir(overlay_dir) if f.endswith(".dtbo")]
#        d.setVar('RPI_KERNEL_DEVICETREE_OVERLAYS', " ".join(overlays))
#}

#-------------------------------------------------------------------------
# Python to add overlays in RPI_KERNEL_DEVICE_TREE
# But Not working
#-------------------------------------------------------------------------
#python __anonymous () {
#    import os
#    overlay_dir = os.path.join(d.getVar('B'), 'arch/arm64/boot/dts/overlays')
#    if os.path.exists(overlay_dir):
#        found_overlays = [f"overlays/{f}" for f in os.listdir(overlay_dir) if f.endswith(".dtbo")]
#        current_overlays = d.getVar('RPI_KERNEL_DEVICETREE_OVERLAYS') or ""
#        # Combine existing overlays with new found overlays, avoiding duplicates
#        overlays_set = set(current_overlays.split()) | set(found_overlays)
#        d.setVar('RPI_KERNEL_DEVICETREE_OVERLAYS', " ".join(sorted(overlays_set)))
#}
...
#    # Merge overlay entries into existing config.txt
...
#        grep -q "^dtoverlay=${overlay}$" $CONFIG_FILE || echo "dtoverlay=${overlay}" >> $CONFIG_FILE
#    done
#
}


#cp -v ${B}/arch/arm64/boot/dts/overlays/*.dtbo ${DEPLOYDIR}/bootfiles/overlays1/
